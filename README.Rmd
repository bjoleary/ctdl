...
The goal of ctdl is to access [clinicaltrials.gov](clinicaltrials.gov) data from R. 
...
We'll also store some known false positives in a vector titled \code{known_fp}. 

```{r known-fp, echo=FALSE}
known_fp <- 
  c(
...
Now we'll filter the \code{studies} tibble based on the search terms: 
